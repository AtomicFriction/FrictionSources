            ...
            if self.dim == 2:
                if self.bound_cond == 'fixed':
                    self.frame = np.setdiff1d(self.bound, np.where(\
                        (self.R[:, 0] - (globals.thickness + self.latt_const) >= 0) & \
                        (self.R[:, 1] - (globals.thickness + self.latt_const) >= 0) & \
                        (self.R[:, 0] + (globals.thickness + self.latt_const) <= self.L - self.latt_const) & \
                        (self.R[:, 1] + (globals.thickness + self.latt_const) <= self.L - self.latt_const)))
