...
from interactions_ghost import AgentForce, SubstrateForce
...
def SimulateAgentVV(status, prot, step, eig_dir, log_dir):
    ...
    if (status == 1):
        ## Updates of the target.
        Agent.ghost_vel += (0.5 * Agent.ghost_acc * globals.dt)
        Agent.V += (0.5 * Agent.A * globals.dt)
        Agent.ghost_pos += (Agent.ghost_vel * globals.dt)
        Agent.R += (Agent.V * globals.dt)
        AgentForce(Agent.R, Agent.ghost_pos, Agent.slider_pos, Subs.R, None)
        Agent.ghost_acc = ((globals.spr_force_gs - (globals.agent_eta * Agent.ghost_vel)) / Agent.mass)
        Agent.A = ((globals.agent_force - (globals.agent_eta * Agent.V)) / Agent.mass)
        Agent.ghost_vel += (0.5 * Agent.ghost_acc * globals.dt)
        Agent.V += (0.5 * Agent.A * globals.dt)
        ## Operation to constrain the target, depends on the user input.
