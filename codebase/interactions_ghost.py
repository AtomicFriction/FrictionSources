...
"""
def AgentForce(agent_pos, ghost_pos, slider_pos, substrate_pos, box):
    """
    ...
    """
    # Individual distances between the 3D components of the agent, ghost and slider.
    diff_ag = agent_pos - ghost_pos # _ag at the end stands for agent-ghost.
    diff_gs = ghost_pos - slider_pos # _gs at the end stands for ghost-slider.
    """
    ...
    """
    norm_ag = np.linalg.norm(diff_ag)
    dR_ag = ((norm_ag - globals.eq_len) / norm_ag) * np.transpose(diff_ag)

    norm_gs = np.linalg.norm(diff_gs)
    dR_gs = ((norm_gs - globals.eq_len) / norm_gs) * np.transpose(diff_gs)
    """
    ...
    spr_force_ag = (- globals.agent_k * dR_ag)
    globals.spr_force_ag = np.reshape(spr_force_ag, (1, 3)) + normal

    spr_force_gs = (- globals.agent_k * dR_gs + globals.agent_k * dR_ag)
    globals.spr_force_gs = np.reshape(spr_force_gs, (1, 3)) + normal

    # Evaluate the total force on the agent. This is where we add the normal force as well.
    globals.agent_force = lj_agent + globals.spr_force_ag
