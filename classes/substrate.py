...
class Substrate():
    def __init__(self):
        ...
        self.V[self.bound] = np.random.normal(0, np.sqrt(boltz*run[0, 0]/self.mass))
        self.V[self.bound] = np.sqrt(self.mass/(2*np.pi*boltz*run[0, 0])) * np.exp(-self.mass*self.V[self.bound]**2/(2*boltz*run[0, 0]))
